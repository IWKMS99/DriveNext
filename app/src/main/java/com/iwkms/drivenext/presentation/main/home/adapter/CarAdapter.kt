...
class CarAdapter(
    ...
    inner class CarViewHolder(private val binding: ItemCarCardBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(car: Car) {
            binding.tvCarModel.text = car.model
            binding.tvCarBrand.text = car.brand
            binding.tvCarPrice.text = "${car.pricePerDay} в день"
            binding.tvTransmission.text = car.transmission
            binding.tvFuelType.text = car.fuelType
            // TODO: Загрузка изображения с помощью Coil
